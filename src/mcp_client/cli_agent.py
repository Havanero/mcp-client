#!/usr/bin/env python3
"""
CLI Agent - Interactive command-line interface for LLM + MCP
...
class CLIAgent:
    """
    Command-line LLM agent with MCP integration
    ...
    def __init__(self, config: AgentConfig):
        ...
        self.logger = logging.getLogger("cli.agent")

    @asynccontextmanager
    async def session(self):
        """Context manager for agent session"""
        # Create LLM client
        llm_client = LLMClientFactory.create_client(self.config.llm)

        # Create orchestrator
        self.orchestrator = MCPOrchestrator(llm_client, self.config.mcp)

        try:
            # Start MCP session
            async with self.orchestrator.session():
    ...
    async def get_status(self) -> str:
        ...
        try:
            ...
            status_lines = [
                ...
                f"🌊 MCP Server: {stats.get('mcp_server', {}).get('status', 'unknown')}",
    ...
    def _print_banner(self):
        ...
        print("=" * 60)
        print("🧠 LLM CLI Agent with MCP Integration")
        print("=" * 60)
        ...
        print("  /debugtools - Debug tool discovery")
    ...
    def _print_help(self):
        ...
        print("  - Ask for file operations, data analysis, etc.")
        print()
        ...
        print("  - 'Read the contents of config.py'")
        print("  - 'What tools are available?'")
        print("  - 'Analyze the code in main.py'")
        print()
    ...
    async def _handle_tools_command(self):
        ...
        try:
            tools = await self.orchestrator.discover_tools()
    ...
    async def _handle_debug_tools_command(self):
        ...
        try:
            debug_info = await self.orchestrator.debug_tools()
            print("\n🔍 Tool Debug Information:")
            print("-" * 40)
            print(f"Tools discovered: {debug_info['tools_discovered']}")
                ...
                while True:
                    try:
                        ...
                        if user_input.startswith("/"):
                            ...
                            elif command == "debugtools":
                                await self._handle_debug_tools_command()
                            else:
                                print(
