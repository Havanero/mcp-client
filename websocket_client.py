        ...
        try:
            response = await self._send_request(
                "initialize",
                {
                    ...
                    "clientInfo": {"name": "websocket-mcp-client", "version": "1.0.0"},
                },
            )
            ...
            if "result" in response:
                ...
                logging.info(
                    f"✅ Connected to: {self.server_info.get('serverInfo', {}).get('name', 'Unknown')}"
                )
                return True
            else:
                logging.error(
                    f"❌ Initialize failed: {response.get('error', 'Unknown error')}"
                )
                return False
