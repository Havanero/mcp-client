...
class HTTPMCPClient:
    ...
    async def _handle_response(self, response: aiohttp.ClientResponse, endpoint: str = "") -> Any:
        ...
        try:
            if response.status >= 400:
                ...
                raise MCPHTTPError(f"HTTP {response.status}: {error_text}", response.status)
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            async with self.context.session.get(health_url) as response:
                ...
                self.logger.info(f"✅ Server health: {health_data.get('status', 'unknown') if health_data else 'unknown'}")
            ...
            try:
                ...
                async with self.context.session.get(info_url) as response:
                    if response.status == 200:
                        self.context.server_info = await self._handle_response(response, "info")
            except:
    ...
    def get_tool_by_name(self, name: str) -> Optional[Dict[str, Any]]:
        ...
        return next((tool for tool in self.context.tools if tool.get('name') == name), None)
    ...
    async def call_mcp_method(self, method: str, params: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.context.session.post(mcp_url, json=request_data) as response:
                data = await self._handle_response(response, "mcp")
                
                if "error" in data:
                    error = data["error"]
                    raise MCPHTTPError(f"MCP Error: {error.get('message', 'Unknown')}", 
...
def create_http_client(base_url: str = "http://localhost:8080", **kwargs) -> HTTPMCPClient:
